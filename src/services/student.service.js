...
export const getMyResultsByClass = (classId, user) => {
  const filter = { student: user._id, institute: user.institute };
  if (classId) filter.class = classId;
  return repo.findMyResults(filter);
};
...
export const getMyReportCard = async (user) => {
  ...
  const [student, institute, results, attendanceRecords] = await Promise.all([
  ...
  return { student, institute, results, attendance, position, generatedAt: new Date() };
};
